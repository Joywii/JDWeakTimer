...
{
    return [[JDTimer alloc] initWithInterval:interval leeway:leeway queue:queue block:block];
}

+ (JDTimer *)timerWithStart:(uint64_t)start leeway:(uint64_t)leeway queue:(dispatch_queue_t)queue block:(dispatch_block_t)block
{
    return [[JDTimer alloc] initWithStart:start leeway:leeway queue:queue block:block];
}
